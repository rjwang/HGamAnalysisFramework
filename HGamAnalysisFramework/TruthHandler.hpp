...
namespace HG {
  ...
  template <class T>
  bool
  TruthHandler::checkEventAndStore(DataVector<T> &cont, std::string name, bool fullCheck)
  {
    // For a full check, check all combinations that may exist in TEvent/TStore
    if (fullCheck) {
      if (checkEventAndStore(cont, m_MxAODName + name))
        return true;
      if (checkEventAndStore(cont, "Deep" + name))
        return true;
      return false;
    }
