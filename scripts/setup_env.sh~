...
[ "${ROOTSYS}x" == "x" ] \
   && echo "Going to set up FAXTools-`showVersions fax | grep default | awk '{print $2}'` from cvmfs" \
   && export RUCIO_ACCOUNT="$USER" \
   && localSetupFAX --quiet 1>/dev/null 2>&1

# RootCore configs
export CERN_USER="$USER"
export ROOTCORE_NCPUS=8
...
alias panda='localSetupPandaClient'
alias vprox='voms-proxy-init -voms atlas'
