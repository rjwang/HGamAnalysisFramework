...
namespace HG {
  ...
  EL::StatusCode JetHandler::initialize(Config &config)
  {
    ...
    if (m_enableBTagging) {
      ...
      for (auto name: m_bTagNames) {
        ...
        for (size_t i = 0; i < m_bTagOPs[name].size(); ++i) {
          ...
          CP_CHECK(m_name, tempSelTool->setProperty("FlvTagCutDefinitionsFileName", bTagCDI        .Data()));
    ...
    if (m_isAFII)
      jetconfig = config.getStr(m_name+".Calibration.ConfigFileAFII");
    ...
    TString jetdef = jetAlgo.Data();
    if (jetAlgo == "AntiKt4EMPFlow") {
      Warning("JetHandler::initialize()", "Initializing JESProvider with AntiKt4EMTopo, since PFlow uncertainties aren't available yet.");
  ...
  xAOD::JetContainer JetHandler::getCorrectedContainer()
  {
    ...
    if (m_correctVertex && m_store->contains<ConstDataVector<xAOD::VertexContainer> >("HGamVertices")) {
      ...
      if (vertex && vertex->vertexType() != xAOD::VxType::PriVtx) {
        ...
        if (m_event->contains<xAOD::CaloClusterContainer>("CaloCalTopoClusters")) {
          static SG::AuxElement::Decorator<int> PVIndex("PVIndex");

          PVIndex(*eventInfo) = vertex->index();
          m_jetOriginTool->modify(shallowContainer);
        } else {
          HG::fatal("No CaloCalTopoClusters found. Either use a more recent DxAOD, or set JetHandler.Calibration.CorrectVertex to NO");
        }
      }
      ...
      if(passJVTCut(jet, m_jvt)) {
        ...
        for (auto name: m_bTagNames) {
          for(size_t i = 0; i < m_bTagOPs[name].size(); i++) {
            ...
            if (isMC()) {
                //Apply eff
                 CP::CorrectionCode result = m_bTagEffTools[name].at(i)->getEfficiency(*jet, effsf);
              if (result == CP::CorrectionCode::Ok)
                jet->auxdata<float>(("Eff_"+name+"_"+m_bTagOPs[name].at(i)).Data()) = effsf;

              //Apply inEff
              result = m_bTagEffTools[name].at(i)->getInefficiency(*jet, effsf);
              if (result == CP::CorrectionCode::Ok)
                jet->auxdata<float>(("InEff_"+name+"_"+m_bTagOPs[name].at(i)).Data()) = effsf;

              //Apply the correct SF if its tagged or untagged.
              if(m_bTagSelTools[name].at(i)->accept(*jet))
                  result = m_bTagEffTools[name].at(i)->getScaleFactor(*jet, effsf);
              else
                  result = m_bTagEffTools[name].at(i)->getInefficiencyScaleFactor(*jet, effsf);
